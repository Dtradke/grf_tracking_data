...
flags.DEFINE_integer('num_games', 5, 'Integer representing the number of games to play.')
...
def main(_):
    ...
    df = pd.DataFrame(
        columns=['GAME_ID', 'TIMESTEP', 'TEAM_0_SCORE', 'TEAM_1_SCORE', 'PLAYER_ID', 'TEAM_ID', 'PLAYER_ROLE', 'LOCATION_X', 'LOCATION_Y',
    ...
    try:
        while True:
            ...
            ball_info = [game_id, count, obs['score'][0], obs['score'][1], -1, -1, -1] + list(obs['ball']) + list(obs['ball_direction']) + [
                False]
            ...
            for player_count in range(left_team.shape[0]):
                ...
                has_ball = True if obs['ball_owned_team'] == 0 and obs['ball_owned_player'] == player_count else False
                player_info = [game_id, count, obs['score'][0], obs['score'][1], player_id, 0, player_role] + list(
                    left_team[player_count]) + [-1] + list(obs['left_team_direction'][player_count]) + [-1] + [has_ball]
            ...
            for player_count in range(right_team.shape[0]):
                ...
                has_ball = True if obs['ball_owned_team'] == 1 and obs['ball_owned_player'] == player_count else False
                player_info = [game_id, count, obs['score'][0], obs['score'][1], player_id, 1, player_role] + list(
                    right_team[player_count]) + [-1] + list(obs['right_team_direction'][player_count]) + [-1] + [
                                  has_ball]
                df.loc[df.shape[0]] = player_info
            ...
            if done:
                ...
                if game_id >= (FLAGS.num_games+FLAGS.game_id):
                    ...
                    logging.warning(f"Games finished, saving data to {save_file_name}...")
